...
import { IClientsDataClientResponse, serializeClientsData } from '@/presentation/utils/serialization.utils';
...
export default async function ClientsRoute(props: ClientsRouteProps) {
  ...
  try {
    ...
    const getClientsDataService: GetClientsDataService = container.get<GetClientsDataService>(TOKENS.GetClientsDataService);
    ...
    const clientsData: IClientsDataResponse = await getClientsDataService.execute(filters);

    // Presentation Layer responsibility: Serialize domain entities for client components
    const serializedResult: IClientsDataClientResponse = serializeClientsData(clientsData);
